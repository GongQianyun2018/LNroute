            ...
            if(recieve_route["Des"]!=self.port):
                ...
                for self_route in self.routetable.route_table[:]:
                    ...
                    if(recieve_route["Des"]==self_route["Des"]):
                        flag=1
                        if(recieve_route["Total_TTLs"]+1<self_route["Total_TTLs"]):#更新路径
                            broadcast_flag=1
                            self.routetable.route_table.remove(self_route)
                            route={"Source":self.port,"Next":recieve_route["Source"],"Fee":self.fee,"Des":recieve_route["Des"],"Total_Fees":recieve_route["Total_Fees"]+recieve_route["Fee"], "Total_TTLs": recieve_route["Total_TTLs"]+1}
...
class routetable:
    def __init__(self,Neigbor_nodes,Node):
        ...
        for neigbor in Neigbor_nodes:
            route={"Source":Node.port,"Next":neigbor.port,"Fee":Node.fee,"Des":neigbor.port,"Total_Fees":0, "Total_TTLs":1}
